...
def main ():
    ...
    folder = "khnpl_alerts/alertsPalawanv2"
    ...
    ft1 = ee.FeatureCollection("projects/cemis-camp/assets/alerts/alertSampleMK" + str(year) + "_001")
    ft2 = ee.FeatureCollection("projects/cemis-camp/assets/alerts/alertSampleMK" + str(year) + "_002")
    ft3 = ee.FeatureCollection("projects/cemis-camp/assets/alerts/alertSampleMK" + str(year) + "_003")
    ft4 = ee.FeatureCollection("projects/cemis-camp/assets/alerts/alertSampleMK" + str(year) + "_004")
    ft5 = ee.FeatureCollection("projects/cemis-camp/assets/alerts/alertSampleMK" + str(year) + "_005")
    ft6 = ee.FeatureCollection("projects/cemis-camp/assets/alerts/alertSampleMK" + str(year) + "_006")
    ft7 = ee.FeatureCollection("projects/cemis-camp/assets/alerts/alertSampleMK" + str(year) + "_007")
    ft8 = ee.FeatureCollection("projects/cemis-camp/assets/alerts/alertSampleMK" + str(year) + "_008")
    ft9 = ee.FeatureCollection("projects/cemis-camp/assets/alerts/alertSampleMK" + str(year) + "_009")

    sample_locations = ft1.merge(ft2).merge(ft3).merge(ft4).merge(ft5).merge(ft6).merge(ft7).merge(ft8).merge(ft9).randomColumn("random",143)
    sample_locations = sample_locations.sort("random")

    # filter for location with alerts
    #sample_locations = sample_locations.filter("total < 100")
    #sample_locations = sample_locations.filter("total > 1000")
    sample_locations = sample_locations.filter("total > 10000")

    beforeDate = ee.Date.fromYMD(year,1,1)
    afterDate = ee.Date.fromYMD(year,12,31)


    label = ee.Image(ee.ImageCollection("projects/servir-mekong/UMD/loss").filterDate(beforeDate,afterDate).first()).divide(100).rename("alert")
    ...
    for i in range(s,500,1):

        print(year, i)


        def addRandomPoints(feature):
            bounds = feature.geometry().buffer(320).bounds()
            points = ee.FeatureCollection.randomPoints(region=bounds, points=1, seed=12*i)
            return points
        ...
        start +=step
        #print(start,step,end,points.size().getInfo())
        ...
        if values[i]<=0.1:
            trainFilePrefix = "/validation/valid10k_"+str(year) + "_" + str(i).zfill(4)
        elif values[i]>0.1 and values[i] <= 0.3:
            trainFilePrefix = "/testing/test10k_"+str(year) + "_"+ str(i).zfill(4)
        else:
            trainFilePrefix = "/training/train10k_"+str(year) + "_"  + str(i).zfill(4)
